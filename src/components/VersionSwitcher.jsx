...
export default function VersionSwitcher() {
  ...
  const onNew = async () => {
    const name = prompt("Name the new version (e.g. General):", "New version");
  ...
  const onDuplicate = async () => {
    const name = prompt(
      "Name the copy (e.g. Acme Corp):",
      current ? `${current.name} copy` : "Copy"
    );
    if (!name || !name.trim()) return;
    const v = await duplicateVersion(name.trim());
    await switchVersion(v.id);
    setOpen(false);
  };

  const onRename = async (v) => {
    const name = prompt("Rename version:", v.name);
    if (name && name.trim() && name.trim() !== v.name) await renameVersion(v.id, name.trim());
  };

  const onDelete = async (v) => {
    if (!confirm(`Delete the “${v.name}” version and all its languages?`)) return;
